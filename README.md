...
![시스템 아키텍처](picture/architecture.png)
...
### 캐시 키 생성
```
키 형식: jargon:v3:{term}:_global
예시: jargon:v3:알고리즘탔다:_global
```

### 성능 최적화
- **전역 캐시**: 같은 단어는 문맥과 무관하게 캐시 히트
- **단순화된 키**: term만으로 빠른 검색을 위한 효율적인 키 구조
- **자동 만료**: TTL로 메모리 효율성 확보

![캐시 히트](picture/cach_hit.png)
*캐시 히트로 인한 즉시 응답*
...
### 성능 지표
- **캐시 히트율**: 95%+ (같은 단어 재사용 시)
- **응답 시간**: 캐시 히트 시 0ms, API 호출 시 2-5초
- **동시 사용자**: 전역 캐시로 확장성 및 효율성 향상
